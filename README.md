...
cargo run -- start --repo . --session-key test --channel discord --thread-id thread-test --requester-user-id EXAMPLE_DISCORD_USER_ID --task-agent-id loop-worker --tick-sec 1 --deliver-openclaw --task-runner-cmd './scripts/rl-task-agent.sh'
